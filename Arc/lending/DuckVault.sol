...
// duckfun.family (alt. duckpad.fun) — DuckVault (Arc)
//
// Lends a token's pool quote asset against the token. On Arc that quote asset is always an ERC-20 (USDC by
// default), so reserves, loans and the buyback all use it directly.
...
contract DuckVault is Initializable, UUPSUpgradeable, LaunchRouting {
    ...
    function linkPool(
        address currency_, bytes32 poolId_, bool tokenIsCurrency0_, uint8 currencyDecimals_
    ) external {
    ...
    function triggerBuyback() external nonReentrant returns (uint256 burned) {
        ...
        burned = _executeV4Swap(
            ...
            currency, token, amount, 0, address(this)
        );
        if (burned > 0) {
