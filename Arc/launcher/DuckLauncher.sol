...
// duckfun.family (alt. duckpad.fun) — DuckLauncher (Arc)
//
// Instant-pool launches on Arc, quoted in an ERC-20 (USDC by default), never raw native. The launch fee and
// the optional instant buy are paid in native USDC; for a USDC launch the instant buy needs no swap.
...
contract DuckLauncher is Initializable, UUPSUpgradeable, Ownable2StepUpgradeable, ReentrancyGuard, LaunchRouting {
    ...
    function initialize(
        ...
        // USDC, Arc's native money, is the curated quote asset seeded here.
        quoteTokens[ARC_USDC] = true;
        emit QuoteTokenAdded(ARC_USDC);
    }
    ...
    // No native rescue: on Arc native USDC is the USDC ERC-20 balance, so rescueERC20(ARC_USDC, ...) covers it.
    ...
    function _setupAndRegister(
        ...
        if (vaultFactory != address(0) && p.vaultBps > 0) {
            ...
            (uint8 quoteDecimals, bool decOk) = _safeDecimals(p.quoteToken);
            if (decOk) {
                try IDuckVaultLinkLocal(vault).linkPool(p.quoteToken, poolId, !tokenIsCurrency1, quoteDecimals) {} catch {}
            }
        }
    ...
    function instantBuy(
        ...
        // Native USDC into the quote asset: a unit conversion for USDC, a USDC route for anything else.
