...
// duckfun.family (alt. duckpad.fun) — DuckReliquify (Arc)
//
...
//
// The migrated token is ALWAYS paired with Arc's canonical USDC (ARC_USDC) -- deliberately, regardless
// of whatever quote asset the old token happened to be paired with. Arc has no native pool currency at
// all (DuckGenesisHook rejects address(0) outright), so ARC_USDC is the direct Arc equivalent of "seed
// with ETH" on the shared tree: one canonical currency for every migration, hardcoded, not a
// per-migration leader choice. Every old token just gets sold for ARC_USDC via LaunchRouting's
// existing routes[] + universalRouter machinery (unmodified, not a new "arbitrary admin-approved
// router" primitive -- see setRoutes below); native itself is used only as that machinery's own
// internal routing intermediate (oldToken -> native -> ARC_USDC), never as a pool currency.
...
import {requireArcChain, ARC_USDC} from "duck-lib/ArcChain.sol";
...
contract DuckReliquify is Initializable, UUPSUpgradeable, Ownable2StepUpgradeable, ReentrancyGuard, LaunchRouting {
    ...
    // Configures how a SPECIFIC token gets sold/bought against native -- reused unmodified from
    ...
    // (a path ending at native) as part of reviewing a proposal, and for ARC_USDC too if no earlier
    // migration already configured a route for it (needed once, shared by every migration -- ARC_USDC
    // is the one, hardcoded, contract-wide quote currency, see file header). This is deliberately NOT a
    // per-migration field -- routes are global per-token on this contract, admin-managed, exactly like
    // every other launch family's routing config.
    ...
    function approveMigration(
        ...
        // Reward currency is always ARC_USDC -- every migration's pool is ARC_USDC-paired,
        // unconditionally (see file header).
        IDuckReliquifyTokenLocal(token).initToken(name_, symbol_, oldSupply, false, metaURI_, v4Hook, ARC_USDC, v4Singleton);
    ...
    function seedPool(uint256 id, uint256 minQuoteOut) external onlyOwner nonReentrant {
        ...
        uint256 quoteRaised = _sellOldTokenForQuote(m.oldToken, m.totalDeposited, minQuoteOut);
    ...
    function _seedInitialLiquidity(uint256 id, Migration storage m, uint256 lpSupply, uint256 quoteRaised) private returns (bytes32 poolId) {
        // ARC_USDC is always the real, final ERC-20 currency here -- it's a genuine ERC-20, so unlike
        // the shared-tree build there's no wrap step needed before settling liquidity.
        address quoteToken = ARC_USDC;
        ...
        if (vault != address(0)) {
            try IDuckVaultLinkLocal(vault).linkPool(quoteToken, poolId, m.newToken == token0, IERC20DecimalsLocal(quoteToken).decimals()) {} catch {}
        }
    }
    ...
    function depositPostSeed(uint256 id, uint256 amount, uint256 minQuoteOut, uint256 minBurnOut) external nonReentrant {
        ...
        uint256 quoteOut = _sellOldTokenForQuote(m.oldToken, received, minQuoteOut);
    ...
    // oldToken -> native -> ARC_USDC. routes[] is always native-anchored (see file header) -- there is
    // no single-hop arbitrary ERC20<->ERC20 path in this codebase, so reaching ARC_USDC (the one,
    // hardcoded, contract-wide quote currency) is always a genuine two-hop compose, using native purely
    // as a routing intermediate -- never as a pool currency.
    function _sellOldTokenForQuote(address oldToken_, uint256 amount, uint256 minQuoteOut) private returns (uint256 quoteOut) {
        if (routes[oldToken_].length == 0) revert SellRouteNotConfigured();
        (uint256 nativeOut, bool ok) = _disposeQuoteToken(oldToken_, amount, 0, address(this));
        if (!ok) revert SellRouteNotConfigured();
        if (routes[ARC_USDC].length == 0) revert SellRouteNotConfigured();
        (uint256 acquired, bool ok2) = _acquireQuoteToken(ARC_USDC, nativeOut, minQuoteOut, address(this));
        if (!ok2) revert SellRouteNotConfigured();
        return acquired;
    }
    ...
    function _buybackAndBurn(Migration storage m, uint256 quoteAmount, uint256 minBurnOut) private returns (uint256 burned) {
        ...
        burned = _executeV4Swap(v4Singleton, v4Hook, FEE_TIER, TICK_SPACING, ARC_USDC, m.newToken, quoteAmount, minBurnOut, address(this));
