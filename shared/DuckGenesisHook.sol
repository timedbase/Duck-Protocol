...
// duckfun.family (alt. duckpad.fun) — DuckGenesisHook
//
...
contract DuckGenesisHook {
    ...
    function unlockCallback(bytes calldata data) external returns (bytes memory) {
        ...
        if (currencyIn == address(0)) {
            IGenesisPoolManagerSwap(msg.sender).settle{value: amountIn}();
        } else {
        ...
        private returns (uint256 remaining)
    {
        ...
        if (currency == address(0)) {
            try IGenesisTokenReward(token).depositHolderReward{value: cut}(cut) {
                remaining = currentRemaining - cut;
            } catch {
                emit HolderRewardSkipped(poolId, cut);
            }
        } else {
            // approve() and depositHolderReward() share one external self-call so a revert from
            // either step is caught rather than bricking the claim -- and since nothing committed,
            // there's no dangling allowance to clear.
    ...
    function _depositVaultCut(address token, address currency, uint256 vaultCut) external {
        ...
        if (currency == address(0)) {
            ...
            IGenesisWETH9(weth).deposit{value: vaultCut}();
            depositCurrency = weth;
        }
        if (!IGenesisERC20(depositCurrency).approve(vault, vaultCut)) revert TransferFailed();
    ...
    function _pay(address currency, address to, uint256 amount) private {
        ...
        if (currency == address(0)) {
            ...
            // fallback would otherwise inflate the caller's gas without bound. Failure still reverts.
            (bool ok,) = to.call{value: amount, gas: 30_000}("");
            if (!ok) revert TransferFailed();
        } else {
            // Capped via a low-level call since a typed call takes no gas option; 100k covers a normal
            // transfer (cold balance slot included) plus a legitimate simple recipient hook.
