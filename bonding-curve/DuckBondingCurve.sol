...
// duckfun.family (alt. duckpad.fun) — DuckBondingCurve
...
contract DuckBondingCurve is Initializable, UUPSUpgradeable, Ownable2StepUpgradeable, LaunchRouting {
    ...
    uint256 private _totalRaisedETH;
    mapping(address => uint256) private _totalRaisedERC;
    uint256 private _totalAccruedFeeETH;
    ...
    function createToken(BaseParams memory p) external payable nonReentrant returns (address token) {
        ...
        // pool's real quote currency -- WETH when the launch's own quote is native, since that's what
        // migration always wraps it into (see BondingCurveMigration._mintV4). Computed the same way
        // here as there so the two can never disagree the way they used to (see BondingCurveMigration's
        // own comment on the bug this replaced). Accepted tradeoff: a token that migrates after a later
        // setDexConfig call keeps the hook/poolManager that were current at its creation, not whatever
        // it actually migrates onto.
        address rewardCurrency_ = p.quoteToken == address(0) ? weth : p.quoteToken;
        ITokenInit(token).initToken(p.name, p.symbol, a.supply, false, p.metaURI, v4Hook, rewardCurrency_, v4Singleton);
        ...
        if (earlyBuy > 0) {
            if (p.quoteToken != address(0)) {
                _safeTransferFrom(p.quoteToken, msg.sender, address(this), earlyBuy);
            }
            _executeBuy(token, msg.sender, earlyBuy, 0);
        }
    }
    ...
    // buyWithNative in reverse: for a non-native-quoted token, sells the curve math out in that
    // quote currency into this contract's own balance, then routes it through the same configured
    // route to pay the seller native ETH. minQuoteOut guards the curve; minNativeOut the swap leg.
        ...
        external nonReentrant
    {
        ...
        if (quoteToken_ == address(0)) revert NativeQuoteNoSwapNeeded();

        (, bool ok) = _disposeQuoteToken(quoteToken_, netQuote, minNativeOut, msg.sender);
        if (!ok) revert RouteUnavailable();
        ...
        private returns (TokenConfig storage tc, uint256 netQuote, uint256 raisedAfter)
    {
        ...
        (_totalRaisedETH, _totalAccruedFeeETH, netQuote, raisedAfter) = BondingCurveMath.executeSell(
            tc, payoutRecipient, amountIn, minQuoteOut, _totalRaisedETH, _totalRaisedERC, _totalAccruedFeeETH, _totalAccruedFeeERC
        );
    }
    ...
    function claimCurveFee(address token_) external nonReentrant {
        ...
        (_totalAccruedFeeETH, creatorCut, platformCut, needsBuyAndBurn) = BondingCurveMath.settleCurveFee(
            tc, tc.creator, amount, _totalAccruedFeeETH, _totalAccruedFeeERC,
            splitsSnapshot, platformToken, platformWallet
        );
    ...
    function emergencyMigrate(address token_) external onlyOwner nonReentrant {
        ...
        uint256 liqTokens;
        (_totalRaisedETH, migrationAmount, liqTokens) = BondingCurveMigration.emergencyMigrate(
    ...
    function rescueETH(address to) external onlyOwner nonReentrant {
        _requireNonZero(to);
        uint256 amount = BondingCurveMath.rescueETH(_totalRaisedETH, _totalAccruedFeeETH, to);
    ...
    function _executeBuy(
        ...
        (_totalRaisedETH, _totalAccruedFeeETH, tokensOut, netQuoteIn, migrationAttemptFailed) =
            BondingCurveMath.executeBuy(
                tc, token_, buyer, quoteIn, minOut,
    ...
    function _doMigrate(TokenConfig storage tc, address token_) private {
        ...
        bytes32 poolId;
        (_totalRaisedETH, poolId) = BondingCurveMigration.migrate(
