...
// duckfun.family (alt. duckpad.fun) — DuckToken
...
contract DuckToken is Initializable, VotesUpgradeable {
    ...
    bool    private _launchPhaseLocked;
    ...
    // ---- holder reward: 5% of hook fees (see DuckHookV4.claimFees), pro-rata and time-weighted
    // over 12h rounds, pushed out in batches. Config is set once at real-pool-creation time (see
    // setRewardConfig); a curve/crowdfund token has no pool until migration/success, so everything
    // here treats rewardHook == address(0) as "not wired up yet, do nothing". ----
    address public rewardCurrency;     // pool's quote currency; address(0) = native
    ...
    address public poolManagerAddr;    // chain's V4 singleton -- the real "liquidity" exclusion,
                                        // since every pool's reserves live there, not in the LP NFT
    // Protocol-wide and not creator-configurable: 0.25% of total supply to be reward-eligible. Baked
    // in so a creator can't set it to 0% (dust positions collecting payouts) or high enough to
    // exclude real small holders. Based on total supply, which initToken sets once and nothing ever
    // changes, so the floor is fixed for the token's lifetime rather than drifting with burns or
    // pool reserves. DEAD and PoolManager are separately excluded (see _isExcludedFromRewards).
    uint256 public constant MIN_HOLDING_BPS = 25;

    // Enumerable holder set -- separate from _holderCountCheckpoints above, which stays untouched so
    // governance's getPastHolderCount keeps working exactly as it does today.
    ...
    // O(1) running total of non-excluded balances, kept in lockstep with every transfer. This --
    // not the sum of per-account entries -- backs _totalBalanceSecondsByRound at close time.
    // Per-account entries settle lazily (see _settleForPayout), so summing them would undercount
    // every holder who never transacted during the round.
    ...
    // schedule. Kept off the _transfer path so an ordinary wallet-to-wallet transfer never absorbs
    // the gas of advancing someone else's payout round.
    uint256 public constant DISTRIBUTION_INTERVAL = 12 hours;
    // The cursor-based design pays out strictly in order however many calls it takes, so batch size
    // only affects how many calls a round needs, never correctness. Set high because processBatch is
    // off the _transfer path: whoever calls it pays for exactly the batch they asked for. See the
    // 500-real-holder fork test for real numbers.
    ...
    // swap-and-pop array before its own bookkeeping is committed. Contract addresses are
    // reward-eligible, so a holder's receive() can run mid-loop on a native payout and would
    // otherwise reenter to walk the same or later holders again. A blocked reentrant attempt just
    ...
    function initToken(
        ...
        _launchPhaseLocked = lockUntilUnlock_;
    ...
    function postLaunchUnlock() external {
        ...
        if (!_launchPhaseLocked) revert LaunchPhaseAlreadyEnded();
        _launchPhaseLocked = false;
    }

    function launchPhaseLocked() external view returns (bool) { return _launchPhaseLocked; }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_launchPhaseLocked) {
            if (from != mintManager && to != mintManager) revert LaunchPhaseTransferRestricted();
        }
    ...
    // Called once by mintManager (the launch contract) the moment a real pool exists. A
    // curve/crowdfund token has no hook/pool at initToken time, so this can't be set earlier.
    function setRewardConfig(address hook_, address currency_, address poolManager_) external {
        ...
        rewardHook = hook_;
        rewardCurrency = currency_; // address(0) is a valid value here (native quote)
    ...
    function _removeHolder(address account) private {
        ...
        // Checked against actual owed weight, not array position, since a not-yet-visited holder can
        // sit anywhere at or past the cursor:
        ...
        //     the cursor already swept, which would orphan whoever lands there. Requiring the
        //     removed slot to be at or ahead of the cursor guarantees the tail lands safely too.
    ...
    // closed round left off, and an idle account transacting mid-payout would forfeit what it was
    // owed. See _settleForPayout.
    ...
    // Same shape as _accrue but for the aggregate _totalEligibleBalance. This is what backs a
    // round's total weight at close time -- not the sum of per-account entries, which lag for any
    // holder who hasn't transacted (they settle lazily in _settleForPayout). This aggregate is
    // correct regardless of who transacted.
    ...
    // bucket rather than the current one. Idempotent. Called from _accrue (so a live transfer during
    // payout doesn't clobber a pending settlement) and from processBatch's loop (so an account that
    // never transacts still gets its true weight rather than a stale one).
    ...
    // hook calls this at claimFees time with its 5% carve-out. Native: send value directly. ERC20:
    // hook approve()s first, this pulls via transferFrom -- same shape as DuckVault.depositFees's
    // caller side.
    function depositHolderReward(uint256 amount) external payable {
        ...
        if (rewardCurrency == address(0)) {
            ...
            if (!IERC20Rescue(rewardCurrency).transferFrom(msg.sender, address(this), amount)) revert TokenRescueFailed();
        }
        roundPool += amount;
    ...
    // so the next round's accrual can't contaminate frozen weights. Runs at the top of processBatch;
    // a no-op if a previous round's batches are unfinished or the interval hasn't elapsed.
    ...
    // balance-seconds recorded at round close. Permissionless, also called on a schedule by
    // DuckKeeper, and never wired into _transfer.
    function processBatch() public nonReentrant {
        ...
        for (uint256 i = _payoutCursor; i < end; ++i) {
            ...
            delete _balanceSecondsByRound[roundId][account]; // consumed strictly before any external
                // call below -- otherwise a holder carrying contract code by payout time could
                // reenter via receive() and read this same nonzero weight twice in one batch.
            if (weight == 0 || _balances[account] < minBalance) continue;

            uint256 amount = (weight * _payoutPool) / _payoutTotalWeight;
            if (amount == 0) continue;
            if (currency == address(0)) {
                // Gas-capped: contract holders are reward-eligible, so an expensive non-reverting
                // receive() could otherwise inflate the caller's gas without bound. 30k covers a
                // plain receive()/simple bookkeeping.
                (bool ok, ) = account.call{value: amount, gas: 30_000}("");
                if (!ok) continue; // a wallet that can't receive native just forfeits this round rather than reverting the batch
            } else {
                // Same reasoning as the native path: rewardCurrency is chosen at launch, not
                // controlled here, and an ERC777-style token would run recipient code. Reentrancy is
                // already blocked by nonReentrant, but an expensive hook could still burn the
                // caller's gas. 100k covers a normal transfer (cold slot included) plus a simple hook.
                (bool ok, bytes memory ret) = currency.call{gas: 100_000}(
                    abi.encodeWithSelector(IERC20Rescue.transfer.selector, account, amount)
                );
